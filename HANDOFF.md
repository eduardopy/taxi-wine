...
- The site is English-only. He may want Spanish-primary or fully bilingual; not yet asked.
...
2. **Spanish + English with Astro's i18n.** Once the English copy is real, mirroring it. The `routing: { prefixDefaultLocale: false }` pattern keeps the English URL clean.
...
- [ ] Whether he wants the site bilingual (English + Spanish) or English-only
...
Also: it's English-only. For a business that lives or dies by Google rankings on tourist queries, multi-language matters more than design polish.
